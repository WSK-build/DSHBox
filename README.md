...
当前版本 **v1.3.0**：多语言版本——界面支持联合国六种官方语言（English / 简体中文 / العربية / Español / Français / Русский），设置新增「语言」选择器（默认跟随系统），并内置「检查更新」自动检测最新版本、引导至官网下载。
...
| 终端 | 多窗口终端、辅助按键栏、控制面板 |
| 设置 | 外观（主题 / 语言）、存储与清理、DSH 更新（在线 / 离线）、检查更新 App、运行环境包导入、移动端适配装配、诊断、关于 |
...
```bash
./gradlew testDebugUnitTest     # 全量 JVM 单测（v1.3.0 共 214 例）
